...
/**
 * The small JSON client the rules read functions share.
 *
 ...
 * violation (409/422) without parsing the message. A rule set that is in play answers 403 to an edit; one in
 * somebody else's workspace answers 404; activating a rule set with errors answers 409.
 */
...
/**
 * Perform a request against the rules API and return its payload.
 *
 * @throws {RuleApiError} when the server answers with a failing status.
 */
export async function request<T>({
    ...
    if (response.status >= 400) {
        throw new RuleApiError(response.status, messageFrom(payload));
    }
