...
/**
 * The small JSON client the prototype and iteration read functions share.
 *
 ...
 * can redirect and flash on success — see `./mutation` for why that matters more in this module than in most.
 */
 ...
 * violation (409/422) without parsing the message. A completed iteration answers 403; one in somebody else's
 * workspace answers 404; a decision already accepted answers 409.
 */
...
/**
 * Perform a request against the prototype and iteration API and return its payload.
 *
 * @throws {PrototypeApiError} when the server answers with a failing status.
 */
export async function request<T>({
    ...
    if (response.status >= 400) {
        throw new PrototypeApiError(response.status, messageFrom(payload));
    }
