...
/**
 * The ordinal of one iteration of a game.
 *
 ...
 * 999 or overwrite the meaning of an earlier one. That rule is enforced in
 * `CreateGameVersion`; this type exists so a number that reaches the domain
 * cannot be zero, negative, or a string that happened to parse.
 */
final readonly class VersionNumber implements Stringable
{
    ...
    {
        if ($value < self::FIRST) {
            throw InvalidVersionNumber::forValue($value);
        }
    ...
    /**
     * The number a game's first version gets.
     */
