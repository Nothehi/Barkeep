...
/**
 * A game design invariant was violated.
 *
 * Every rule the module enforces reports the HTTP status it should surface
 * as, so the delivery layer can translate the whole family with one renderer
 * instead of catching each exception individually. That renderer lives in
 * `GameDesignServiceProvider::configureExceptionRendering()`; it is named
 * here in prose rather than in a `@see` tag so the domain keeps no import
 * pointing back up at the layers above it.
 */
abstract class GameRuleViolation extends DomainException
{
    /**
