...
/**
 * Reword an end condition, attach a condition to it, or change when it is checked.
 *
 ...
 */
final class UpdateGameEndCondition
{
    ...
    public function handle(User $actor, GameEndCondition $outcome, OutcomeData $data): GameEndCondition
    {
        ...
        if ($data->name !== null && $data->name !== $outcome->name) {
            if ($this->structure->ruleSetHasGameEndConditionNamed($ruleSet, $data->name, $outcome->getKey())) {
                throw RuleNameIsTaken::forGameEndCondition($data->name);
            }
