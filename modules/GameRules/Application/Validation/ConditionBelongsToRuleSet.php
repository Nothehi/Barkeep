...
/**
 * Requires that a condition id names one of this rule set's own conditions.
 *
 * A rule object rather than an `exists:rule_conditions,id,rule_set_id,…` clause, so that
 * "which conditions belong to this rule set" has one definition — the one the
 ...
 */
final class ConditionBelongsToRuleSet implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        if (! is_string($value) || $value === '') {
            $fail(__('Choose a condition.'));

            return;
        }

        if (! app(RuleCatalogue::class)->ruleSetHasCondition($this->ruleSet, $value)) {
            $fail(__('That condition belongs to a different rule set.'));
        }
    }
}
