...
/**
 * Requires that a phase id names one of this rule set's own phases.
 *
 * A rule object rather than an `exists:game_phases,id,rule_set_id,…` clause, so that
 * "which phases belong to this rule set" has one definition — the one the
 ...
 */
final class PhaseBelongsToRuleSet implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        if (! is_string($value) || $value === '') {
            $fail(__('Choose a phase.'));

            return;
        }

        if (! app(RuleCatalogue::class)->ruleSetHasPhase($this->ruleSet, $value)) {
            $fail(__('That phase belongs to a different rule set.'));
        }
    }
}
