...
/**
 * Requires that a trigger id names one of this rule set's own triggers.
 *
 * A rule object rather than an `exists:rule_triggers,id,rule_set_id,…` clause, so that
 * "which triggers belong to this rule set" has one definition — the one the
 ...
 */
final class TriggerBelongsToRuleSet implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        if (! is_string($value) || $value === '') {
            $fail(__('Choose a trigger.'));

            return;
        }

        if (! app(RuleCatalogue::class)->ruleSetHasTrigger($this->ruleSet, $value)) {
            $fail(__('That trigger belongs to a different rule set.'));
        }
    }
}
