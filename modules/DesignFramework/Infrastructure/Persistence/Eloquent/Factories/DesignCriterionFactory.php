...
/**
 * @extends Factory<DesignCriterion>
 */
class DesignCriterionFactory extends Factory
{
    /**
     * The model the factory builds.
     *
     * @var class-string<DesignCriterion>
     */
    protected $model = DesignCriterion::class;

    /**
     * Define the model's default state.
     *
     * Filed under no phase and published, which is the shortest thing a test can ask for.
     * Content under a phase says so through {@see inPhase()}, because which phase a criterion
    ...
    /**
     * Append the criterion to a version, filed under no phase.
     */
    public function inVersion(FrameworkVersion $version): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            'phase_id' => null,
            'position' => DesignCriterion::query()
    ...
    /**
     * Append the criterion to a phase, taking the version from it.
     *
    ...
    public function inPhase(DesignPhaseDefinition $phase): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            'phase_id' => $phase->id,
            'position' => DesignCriterion::query()->where('phase_id', $phase->id)->count() + 1,
        ]);
    }

    /**
     * Give the criterion a specific title, deriving its address to match.
     */
    ...
    /**
     * Place the criterion at a specific point among its siblings.
     */
    ...
    /**
     * Indicate that the criterion is still being written, so designers cannot see it.
     */
    ...
    /**
     * Indicate that the criterion has been dropped from the methodology.
     */
