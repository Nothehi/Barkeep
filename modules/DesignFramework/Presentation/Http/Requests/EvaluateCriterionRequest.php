...
class EvaluateCriterionRequest extends FrameworkRequest
{
     ...
     *
     * `recordProgress` covers all four kinds of write together, because the rule they share is the
     * whole rule: the game must be open and the adoption must be active.
     */
     ...
     *
     * The criterion is absent because it is not an input: it comes from the resolved route binding,
     * which looked it up through the version this game adopted.
     *
     * "Not evaluated" is refused. It is the state a criterion is in before anybody acts, and
    ...
    {
        return [
            'status' => $this->criterionRatingRules(),
     ...
     */
    public function toData(): EvaluateCriterionData
    {
        return EvaluateCriterionData::fromArray($this->validated());
    }
